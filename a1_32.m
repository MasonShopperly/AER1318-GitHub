...
clear; clc;
[gamma, R, T01, p01, S_star, x_shock, j, x] = setupSimulation();
S = defineAreaFunction();
[M, M_post_shock] = initializeMachNumber(j, x, gamma);
[M, p] = solveTransonicFlow(gamma, S, S_star, x, M, p01);
plotResults(x, M, p);

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% Supporting functions
function [gamma, R, T01, p01, S_star, x_shock, j, x] = setupSimulation()
...
function [M, M_post_shock] = initializeMachNumber(j, x, gamma)
    M = zeros(1, j);
    M_post_shock = NaN; 
    for i = 1:j
        if x(i) < 5
        ...
        elseif x(i) >= 5 && x(i) < 7
            ...
            if isnan(M_post_shock)
                M_post_shock = calculatePostShockMach(M(i-1), gamma);
            end
...
function [M, p] = solveTransonicFlow(gamma, S, S_star, x, M, p01)
